...
{#if shouldShow}
	...
	>
		A couple of quick safety questions.<br />
	</h2>
	...
	>
		<div>
			<h3 class="font-medium text-gray-800 mb-3">Are you 18 or older?</h3>
			<div class="flex gap-3">
				<button
					...
					on:click={() => (over18 = true)}>Yes</button
				>
				<button
					type="button"
					class="{pillClass(
					...
					on:click={() => (over18 = false)}>No</button
				>
			</div>
		</div>

		<div>
			<h3 class="font-medium text-gray-800 mb-3">Are you currently pregnant?</h3>
			<div class="flex gap-3">
				<button
					type="button"
					class="{pillClass(
					...
					on:click={() => (pregnant = true)}>Yes</button
				>
				<button
					type="button"
					class="{pillClass(
					...
					on:click={() => (pregnant = false)}>No</button
				>
			</div>
		</div>
		...
		<div>
			<h3 class="font-medium text-gray-800 mb-3">
				Are you currently taking any prescription medications or other supplements?
			</h3>
			<div class="flex gap-3">
				<button
					type="button"
					...
					on:click={() => (medications = true)}>Yes</button
				>
				<button
					type="button"
					class="{pillClass(
					...
					on:click={() => (medications = false)}>No</button
				>
			</div>
		</div>
	</div>
	...
	>
		{#if !allAnswered}
			<span
				>Chronobiotics (melatonin) aren't right for everyone — we just need to check a few things
				first.</span
			>
		{:else if disqualified}
			<span in:fade={{ duration: 500 }}
				>We'll leave chronobiotics out of your plan — bright light timing is still included.</span
			>
		{:else}
			<span in:fade={{ duration: 500 }}
				>Thanks — we'll go ahead and include general chronobiotic guidance in your plan.</span
			>
		{/if}
	</p>
