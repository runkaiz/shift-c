...
{#if shouldShow}
	...
	>
		{$_('survey.idealSchedule.title')}<br />
	</h2>
	<div class="relative max-w-2xl space-x-8 my-16" out:fade={{ duration: 400 }}>
		{#if step === 0}
			...
			>
				<h3 class="font-medium text-gray-800">{$_('survey.idealSchedule.wakeQuestion')}</h3>
		...
		{:else if step === 1}
			...
			>
				<h3 class="font-medium text-gray-800">{$_('survey.idealSchedule.sleepQuestion')}</h3>
	...
	>
		{#if step === 0}
			<span out:fade={{ duration: 500 }}>
				{$_('survey.idealSchedule.wakeHintLine1')}
				<br />
				{$_('survey.idealSchedule.wakeHintLine2')}
			</span>
		{:else if step === 1}
			<span in:fade={{ duration: 500 }}>
				{$_('survey.idealSchedule.sleepHintLine1', {
					values: { time: calcBedtime(data.goal.wakeup) }
				})}
				<br />
				{$_('survey.idealSchedule.sleepHintLine2')}
			</span>
		{/if}
	</p>
